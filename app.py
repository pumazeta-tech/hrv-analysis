...
# =============================================================================
# FUNZIONI PER SPLITTING MULTI-GIORNO MIGLIORATE
# =============================================================================

def detect_sleep_periods(rr_intervals, start_datetime):
    """Identifica automaticamente i periodi di sonno basandosi sui pattern HRV"""
    if len(rr_intervals) < 1000:
        return []
    
    sleep_periods = []
    window_size = 30 * 60 * 1000  # 30 min in ms
    current_pos = 0
    
    while current_pos + window_size < len(rr_intervals):
        window_end = min(current_pos + window_size, len(rr_intervals))
        window_rr = rr_intervals[current_pos:window_end]
        
        if len(window_rr) < 10:
            current_pos += window_size // 2
            continue
            
        hr_mean = 60000 / np.mean(window_rr)
        
        # Calcolo RMSSD più robusto
        if len(window_rr) > 1:
        ...
        # Criteri più flessibili per identificare il sonno
        is_sleep_candidate = (hr_mean < 68 and rmssd > 25)  # Criteri più larghi
        
        if is_sleep_candidate:
            # Verifica che non sia un falso positivo (controlla finestre adiacenti)
            sleep_periods.append({
                'start_idx': current_pos,
                'end_idx': window_end,
                'start_time': start_datetime + timedelta(seconds=sum(rr_intervals[:current_pos])/1000),
                'end_time': start_datetime + timedelta(seconds=sum(rr_intervals[:window_end])/1000),
                'hr_mean': hr_mean,
                'rmssd': rmssd
            })
...
def split_by_days(rr_intervals, start_datetime):
    """Divide il flusso continuo IBI in giorni separati - VERSIONE MIGLIORATA"""
    total_duration_ms = sum(rr_intervals)
    total_hours = total_duration_ms / (1000 * 3600)
    
    # Se la registrazione è inferiore a 24 ore, considera solo un giorno
    if total_hours <= 30:
        return [{
            'day_number': 1,
            'start_idx': 0,
            'end_idx': len(rr_intervals),
            'start_time': start_datetime,
            'end_time': start_datetime + timedelta(seconds=total_duration_ms/1000),
            'rr_intervals': rr_intervals
        }]
    
    # Prova prima con il rilevamento automatico del sonno
    sleep_periods = detect_sleep_periods(rr_intervals, start_datetime)
    
    if sleep_periods:
        days_data = []
        current_start = 0
        
        for i, sleep in enumerate(sleep_periods):
            day_end = sleep['end_idx']
            
            if day_end - current_start > 1800:  # Almeno 30 minuti di dati
                day_data = {
                    'day_number': len(days_data) + 1,
                    'start_idx': current_start,
                    'end_idx': day_end,
                    'start_time': start_datetime + timedelta(seconds=sum(rr_intervals[:current_start])/1000),
                    'end_time': start_datetime + timedelta(seconds=sum(rr_intervals[:day_end])/1000),
                    'rr_intervals': rr_intervals[current_start:day_end]
                }
                days_data.append(day_data)
                current_start = day_end
        
        # Aggiungi l'ultimo giorno
        if current_start < len(rr_intervals) - 1800:
            day_data = {
                'day_number': len(days_data) + 1,
                'start_idx': current_start,
                'end_idx': len(rr_intervals),
                'start_time': start_datetime + timedelta(seconds=sum(rr_intervals[:current_start])/1000),
                'end_time': start_datetime + timedelta(seconds=sum(rr_intervals)/1000),
                'rr_intervals': rr_intervals[current_start:]
            }
            days_data.append(day_data)
    ...
    # Fallback: dividi ogni 24 ore
    return split_every_24h(rr_intervals, start_datetime)

def split_every_24h(rr_intervals, start_datetime):
    """Divide forzatamente ogni 24 ore"""
    total_duration_ms = sum(rr_intervals)
    total_hours = total_duration_ms / (1000 * 3600)
    
    days_data = []
    
    for day in range(int(total_hours // 24) + 1):
        start_ms = day * 24 * 3600 * 1000
        end_ms = (day + 1) * 24 * 3600 * 1000
        ...
        for i, rr in enumerate(rr_intervals):
            cumulative += rr
            if cumulative >= start_ms and start_idx == 0:
                start_idx = i
            if cumulative >= end_ms and end_idx == len(rr_intervals):
                end_idx = i
                break
        
        if end_idx - start_idx > 1800:  # Almeno 30 minuti di dati
            day_data = {
                'day_number': day + 1,
                'start_idx': start_idx,
                'end_idx': end_idx,
                'start_time': start_datetime + timedelta(hours=24*day),
                'end_time': start_datetime + timedelta(hours=24*(day+1)),
                'rr_intervals': rr_intervals[start_idx:end_idx]
            }
            days_data.append(day_data)
    
    return days_data if days_data else [{
        'day_number': 1,
        'start_idx': 0,
        'end_idx': len(rr_intervals),
        'start_time': start_datetime,
...
def create_day_selection_interface(days_data):
    """Crea interfaccia per selezionare/confermare i giorni identificati"""
    st.header("📅 Identificazione Automatica Giorni")
    
    if not days_data or len(days_data) == 0:
        st.info("📊 **Registrazione singola giornata** - Analisi completa di tutti i dati")
        return [{
            'day_number': 1,
            'start_idx': 0,
            'end_idx': days_data[0]['end_idx'] if days_data else 0,
            'start_time': days_data[0]['start_time'] if days_data else datetime.now(),
            'end_time': days_data[0]['end_time'] if days_data else datetime.now(),
            'rr_intervals': days_data[0]['rr_intervals'] if days_data else []
        }]
    
    st.success(f"✅ Identificati {len(days_data)} giorni potenziali")
    
    selected_days = []
    
    for i, day in enumerate(days_data):
        with st.expander(f"📅 Giorno {i+1} - {day['start_time'].strftime('%d/%m %H:%M')} → {day['end_time'].strftime('%d/%m %H:%M')}", expanded=True):
            
            col1, col2, col3 = st.columns(3)
            
            with col1:
                st.write(f"**Durata:** {len(day['rr_intervals'])} battiti")
                st.write(f"**Ore:** {(day['end_time'] - day['start_time']).total_seconds()/3600:.1f}h")
            
            with col2:
                if len(day['rr_intervals']) > 10:
                    metrics = calculate_real_hrv_metrics(day['rr_intervals'])
                    if metrics:
                        st.write(f"**HR:** {metrics['hr_mean']:.1f} bpm")
                        st.write(f"**SDNN:** {metrics['sdnn']:.1f} ms")
            
            with col3:
                include_day = st.checkbox(f"Includi Giorno {i+1}", value=True, key=f"day_{i}")
                if include_day:
                    selected_days.append(day)
    
    return selected_days if selected_days else days_data

# =============================================================================
# FUNZIONE PER CREARE GRAFICO CON ORE REALI - VERSIONE MIGLIORATA
# =============================================================================
...
# =============================================================================
# GRAFICO LINEARE SDNN, RMSSD, HR - SEMPLICE E CHIARO
# =============================================================================

def create_simple_hrv_trend_plot(metrics, start_datetime, end_datetime):
    """Crea un grafico lineare semplice e chiaro di SDNN, RMSSD e HR"""
    duration_hours = metrics['our_algo']['recording_hours']
    
    # Crea timeline più realistica
    time_points = [start_datetime + timedelta(hours=x) for x in range(int(duration_hours) + 1)]
    if len(time_points) < 2:
        time_points = [start_datetime, end_datetime]
    
    # Valori base dalle metriche calcolate
    base_sdnn = metrics['our_algo']['sdnn']
    base_rmssd = metrics['our_algo']['rmssd']
    base_hr = metrics['our_algo']['hr_mean']
    
    # Genera trend realistici con variazioni circadiane
    sdnn_values = []
    rmssd_values = []
    hr_values = []
    ...
    for i, time_point in enumerate(time_points):
        ...
        # Variazione circadiana - SDNN e RMSSD più alti di notte
        if 2 <= hour <= 6:  # Notte profonda
            circadian_factor = 1.2
            hr_factor = 0.9
        elif 7 <= hour <= 9:  # Mattina
        ...
        # Aggiungi variazione casuale moderata
        random_variation = np.random.normal(0, 0.1)
        
        sdnn_val = base_sdnn * circadian_factor * (1 + random_variation)
        rmssd_val = base_rmssd * circadian_factor * (1 + random_variation)
        hr_val = base_hr * hr_factor * (1 + random_variation * 0.5)
        
        # Mantieni valori nei range fisiologici
        sdnn_values.append(max(20, min(120, sdnn_val)))
        rmssd_values.append(max(10, min(100, rmssd_val)))
        hr_values.append(max(50, min(100, hr_val)))
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate='<b>%{x|%H:%M}</b><br>SDNN: %{y:.1f} ms<extra></extra>'
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate='<b>%{x|%H:%M}</b><br>RMSSD: %{y:.1f} ms<extra></extra>'
    ))
    
    # HR - linea verde (secondo asse Y)
    fig.add_trace(go.Scatter(
        x=time_points, 
        y=hr_values, 
        mode='lines+markers',
        ...
        yaxis='y2',
        hovertemplate='<b>%{x|%H:%M}</b><br>HR: %{y:.1f} bpm<extra></extra>'
    ))
    
    fig.update_layout(
        title="📊 Andamento SDNN, RMSSD e Frequenza Cardiaca",
        ...
        yaxis2=dict(
            ...
            overlaying='y',
            side='right',
            range=[40, 100]  # Range fisiologico per HR
        ),
        height=500,
        showlegend=True,
        xaxis=dict(
...
# =============================================================================
# FUNZIONI PER COMPARAZIONE MULTI-GIORNO
# =============================================================================

def create_multi_day_comparison(user_profile, days_back=3):
    """Crea comparazione tra gli ultimi N giorni di analisi"""
    analyses = get_user_analyses(user_profile)
    
    if len(analyses) < 2:
        st.info("📅 **Servono almeno 2 analisi per fare comparazioni**")
        return None
    
    recent_analyses = analyses[-days_back:]
    
    if len(recent_analyses) < 2:
        st.info("📅 **Servono almeno 2 analisi recenti per il confronto**")
        return None
    
    st.header("📊 Comparazione Multi-Giorno")
    
    comparison_data = []
    for i, analysis in enumerate(recent_analyses):
        day_label = f"Giorno {i+1}" if i < 3 else f"Giorno {i+1}"
        comparison_data.append({
            'day': day_label,
            'date': analysis['start_datetime'].strftime('%d/%m'),
            'sdnn': analysis['metrics']['sdnn'],
            'rmssd': analysis['metrics']['rmssd'],
            'hr_mean': analysis['metrics']['hr_mean'],
            'coherence': analysis['metrics']['coherence'],
            'total_power': analysis['metrics']['total_power'],
            'lf_hf_ratio': analysis['metrics']['lf_hf_ratio']
        })
    
    fig = go.Figure()
    
    metrics_to_plot = ['sdnn', 'rmssd', 'hr_mean', 'coherence']
    colors = ['#3498db', '#e74c3c', '#2ecc71', '#f39c12']
    
    for i, metric in enumerate(metrics_to_plot):
        values = [day[metric] for day in comparison_data]
        fig.add_trace(go.Scatter(
            x=[day['date'] for day in comparison_data],
            y=values,
            mode='lines+markers',
            name=metric.upper(),
    ...
    fig.update_layout(
        title="📈 Trend Metriche HRV - Confronto Multi-Giorno",
        xaxis_title="Giorni",
        yaxis_title="Valori",
        height=400,
        showlegend=True
    ...
    st.subheader("📋 Dettaglio Comparazione")
    ...
    for day in comparison_data:
        comparison_table_data.append([
            ...
            f"{day['sdnn']:.1f} ms",
            f"{day['rmssd']:.1f} ms", 
            f"{day['hr_mean']:.1f} bpm",
            f"{day['coherence']:.1f}%",
            f"{day['total_power']:.0f} ms²",
            f"{day['lf_hf_ratio']:.2f}"
        ])
    
    if len(comparison_data) >= 2:
        st.subheader("📈 Variazioni Percentuali")
        
        last_day = comparison_data[-1]
        previous_day = comparison_data[-2]
        
        changes = {
            'SDNN': ((last_day['sdnn'] - previous_day['sdnn']) / previous_day['sdnn']) * 100,
            'RMSSD': ((last_day['rmssd'] - previous_day['rmssd']) / previous_day['rmssd']) * 100,
            'HR': ((last_day['hr_mean'] - previous_day['hr_mean']) / previous_day['hr_mean']) * 100,
            'Coerenza': ((last_day['coherence'] - previous_day['coherence']) / previous_day['coherence']) * 100
        }
        
        col1, col2, col3, col4 = st.columns(4)
        
        for col, (metric, change) in zip([col1, col2, col3, col4], changes.items()):
            with col:
                if "HR" in metric:
                    if change < 0:
                        st.success(f"**{metric}**\n▼ {abs(change):.1f}%")
...
def analyze_trends(comparison_data, user_profile):
    """Analizza le tendenze nei dati multi-giorno"""
    if len(comparison_data) < 2:
        return
    
    st.header("🎯 Analisi Tendenza e Progressi")
    ...
    for metric in ['sdnn', 'rmssd', 'hr_mean', 'coherence']:
        values = [day[metric] for day in comparison_data]
        trend = "miglioramento" if (values[-1] > values[0] and metric != 'hr_mean') else "peggioramento"
        
        if metric == 'sdnn':
            evaluation = get_sdnn_evaluation(values[-1], gender, age)
        elif metric == 'rmssd':
            evaluation = get_rmssd_evaluation(values[-1], gender, age)
        elif metric == 'hr_mean':
            evaluation = get_hr_evaluation(values[-1], age)
            trend = "miglioramento" if values[-1] < values[0] else "peggioramento"
        else:
            evaluation = get_coherence_evaluation(values[-1], age)
    ...
    with col1:
        st.subheader("📈 Tendenza Generale")
        positive_changes = sum(1 for m in metrics_analysis if m['trend'] == 'miglioramento')
        
        if positive_changes >= 3:
            st.success("**🎉 Progresso Eccellente!** La maggior parte delle metriche mostra miglioramento.")
        elif positive_changes >= 2:
            st.info("**📊 Progresso Buono!** Alcune metriche mostrano miglioramento.")
        else:
            st.warning("**⚠️ Attenzione!** La maggior parte delle metriche mostra peggioramento.")
    
    with col2:
        st.subheader("🎯 Raccomandazioni Trend")
        if any(m['trend'] == 'peggioramento' for m in metrics_analysis if m['metric'] in ['SDNN', 'RMSSD']):
            st.write("• **Migliorare recupero**: Più sonno e tecniche rilassamento")
        if any(m['trend'] == 'peggioramento' for m in metrics_analysis if m['metric'] == 'HR_MEAN'):
            st.write("• **Ridurre stress**: Più pause attive durante il giorno")
        if all(m['trend'] == 'miglioramento' for m in metrics_analysis):
            st.write("• **Mantenere routine**: Continuare con le buone abitudini")

# =============================================================================
# INTERFACCIA PRINCIPALE STREAMLIT - VERSIONE MIGLIORATA
# =============================================================================
...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            for line in lines:
                if line.strip():
                    try:
                        ...
                        # Prova a estrarre data/ora
                        if not start_datetime:
                            file_datetime = extract_datetime_from_content(line)
                            if file_datetime:
                                start_datetime = file_datetime
                        continue
            ...
            if not start_datetime:
                start_datetime = datetime.now()
                st.warning("⚠️ Data/ora non rilevata dal file - usando data corrente")
            
            # SPLITTING MULTI-GIORNO MIGLIORATO
            st.header("📅 Analisi Multi-Giorno")
            
            days_data = split_by_days(rr_intervals, start_datetime)
            selected_days = create_day_selection_interface(days_data)
            
            if selected_days:
                st.header("📊 Analisi Giorni Selezionati")
                
                all_day_metrics = []
                
                for i, day in enumerate(selected_days):
                    with st.expander(f"📅 {day['start_time'].strftime('%A %d/%m')} - Giorno {i+1}", expanded=True):
                        
                        day_metrics = calculate_real_hrv_metrics(day['rr_intervals'])
                        if day_metrics:
                            day_metrics['recording_hours'] = (day['end_time'] - day['start_time']).total_seconds() / 3600
                            all_day_metrics.append({
                                'day_label': f"Giorno {i+1}",
                                'date': day['start_time'].strftime('%d/%m'),
                                'weekday': day['start_time'].strftime('%A'),
                                'metrics': day_metrics
                            })
                            
                            # Mostra metriche del giorno
                            col1, col2, col3, col4 = st.columns(4)
                            age = st.session_state.user_profile['age']
                            gender = st.session_state.user_profile['gender']
                            
                            with col1:
                                st.metric("SDNN", f"{day_metrics['sdnn']:.1f} ms", 
                                         delta=f"{get_sdnn_evaluation(day_metrics['sdnn'], gender, age)}")
                            with col2:
                                st.metric("RMSSD", f"{day_metrics['rmssd']:.1f} ms",
                                         delta=f"{get_rmssd_evaluation(day_metrics['rmssd'], gender, age)}")
                            with col3:
                                st.metric("HR", f"{day_metrics['hr_mean']:.1f} bpm",
                                         delta=f"{get_hr_evaluation(day_metrics['hr_mean'], age)}")
                            with col4:
                                st.metric("Coerenza", f"{day_metrics['coherence']:.1f}%",
                                         delta=f"{get_coherence_evaluation(day_metrics['coherence'], age)}")
                            
                            # GRAFICO LINEARE PER OGNI GIORNO
                            st.subheader("📈 Andamento Giornaliero")
                            daily_trend_fig = create_simple_hrv_trend_plot(
                                {'our_algo': day_metrics}, 
                                day['start_time'], 
                                day['end_time']
                            )
                            st.plotly_chart(daily_trend_fig, use_container_width=True)
                
                # COMPARAZIONE TRA GIORNI
                if len(all_day_metrics) >= 2:
                    st.header("📈 Comparazione tra Giorni")
                    
                    comparison_data = []
                    for day_data in all_day_metrics:
                        comparison_data.append({
                            'day': day_data['day_label'],
                            'date': day_data['date'],
                            'sdnn': day_data['metrics']['sdnn'],
                            'rmssd': day_data['metrics']['rmssd'],
                            'hr_mean': day_data['metrics']['hr_mean'],
                            'coherence': day_data['metrics']['coherence'],
                            'total_power': day_data['metrics']['total_power'],
                            'lf_hf_ratio': day_data['metrics']['lf_hf_ratio']
                        })
                    ...
                    # Analisi tendenze
                    analyze_trends(comparison_data, st.session_state.user_profile)
            
            else:
                # ANALISI SINGOLO GIORNO (backup)
                st.header("⏰ Analisi Singola Giornata")
                
                # Calcola metriche per tutti i dati
                real_metrics = calculate_real_hrv_metrics(rr_intervals)
                
                if real_metrics:
                    metrics = {'our_algo': real_metrics}
                    metrics['our_algo']['recording_hours'] = sum(rr_intervals) / (1000 * 3600)
                else:
                    ...
                    metrics = {
                        'our_algo': {
                            ...
                            'hf': 450.0,
                            'lf_hf_ratio': 1.33,
                            'recording_hours': sum(rr_intervals) / (1000 * 3600)
                        }
                    }
                
                end_datetime = start_datetime + timedelta(seconds=sum(rr_intervals)/1000)
                selected_range = f"{metrics['our_algo']['recording_hours']:.1f} ore"
                ...
                st.subheader("📈 Andamento SDNN, RMSSD e Frequenza Cardiaca")
                trend_fig = create_simple_hrv_trend_plot(metrics, start_datetime, end_datetime)
